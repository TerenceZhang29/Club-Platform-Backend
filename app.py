...
def google_login():
    ...
    if response.status_code == 200:
        data = response.json() # Contains user info related to backend
        # If user exists in the database, create a session for the user
        # If not, create a new user
        return json.dumps({"success": True, "data": data['aud']})
    else:
        print(response.status_code)
        return json.dumps({"success": False, "data": "Invalid id_token"})
